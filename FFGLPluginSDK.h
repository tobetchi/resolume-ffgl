...
public:
  ...
  virtual FFResult SetTime(double time) { return FF_FAIL; }
	...
	virtual const char * GetShortName() { return 0; }
	...
	/// \return		The default implementation always returns FF_SUCCESS.
	virtual unsigned int Connect() { return FF_SUCCESS; }
	...
	/// \return		The default implementation always returns FF_SUCCESS.
	virtual unsigned int Disconnect() { return FF_SUCCESS; }

	/// Called when plugin viewport size is changed
	/// \param		vp Pointer to a FFGLViewportStruct structure (see the definition in FFGL.h and
	///				the description in the FFGL specification).
	/// \return		The default implementation always returns FF_SUCCESS.
	virtual unsigned int Resize(const FFGLViewportStruct *vp) { return FF_SUCCESS; }
